testthat::test_that("checksum is enforced for downloads and cached reads", {
  ...
  # 3) Ensure again with correct sha: it should detect mismatch, re-download, and succeed
  ...
  # 4) If we demand a WRONG sha, we should ERROR with a checksum mismatch (after re-download)
